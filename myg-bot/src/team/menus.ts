...
export async function handleTeamSelectMenu(inter: StringSelectMenuInteraction) {
  ...
  if (parsed) {
    ...
    if (parsed.kind === "ROLE") {
      await inter.deferUpdate();
      const lobby = await prisma.lobby.findUnique({ where: { id: parsed.lobbyId }, include: { teamsList: true, participants: true } });
      if (!lobby) return;
      const gm = inter.member as GuildMember | null;
      if (!isRespoOrCreator(gm, lobby.createdBy, process.env.ROLE_RESPO_ID!)) return;
      ...
      const assignedIds = new Set(
        (await prisma.teamMember.findMany({ where: { team: { lobbyId: lobby.id } } })).map((m) => m.lobbyParticipantId)
      );
      ...
      const playerMenu = new (StringSelectMenuBuilder as any)()
        .setCustomId(tbPlayerSelectId(lobby.id, role))
        .setPlaceholder(
          `Choisir joueur (${role}) → ${lobby.teamsList.find((t) => t.id === teamId)?.name ?? "Team ?"}`
        )
        .addOptions(pool.map((p) => ({ label: p.display, value: `${teamId}:${p.id}` })));

      const rowPlayers = new (ActionRowBuilder as any)().addComponents(playerMenu);
  ...
  if (id.startsWith("TB:NAME:TEAMSEL:")) {
    ...
    const modal = new ModalBuilder()
      .setCustomId(`TB:NAME:${lobbyId}:${teamId}`)
      .setTitle("Renommer l'équipe");
  ...
  if (id.startsWith("TB:CAP:TEAMSEL:")) {
    ...
    const sel = new (StringSelectMenuBuilder as any)()
    ...
    const row = new (ActionRowBuilder as any)().addComponents(sel);

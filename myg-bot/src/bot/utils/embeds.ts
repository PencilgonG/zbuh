...
import type { LobbyState, MatchItem } from "./lobby.js";
...
export function lobbyEmbed(opts: {
  ...
  const e = new EmbedBuilder()
    ...
    .setDescription(
      `Mode **${opts.mode}** • Équipes: **${opts.teamCount}**\n` +
...
export function teamBuilderEmbed(state: LobbyState) {
  ...
  const e = new EmbedBuilder()
    ...
    .setDescription(
      `Mode **${state.mode}** • Équipes **${state.teamCount}** • ` +
  ...
  for (const t of teams) {
    const lines: string[] = [];
    (["Top", "Jungle", "Mid", "ADC", "Support"] as CoreRole[]).forEach((r) => {
...
export function lineupEmbed(state: LobbyState) {
  const e = new EmbedBuilder()
...
export function matchEmbed(state: LobbyState, m: MatchItem) {
  const blue =
    state.teams.find((t) => t.number === m.blueTeam)?.name ??
    `Team ${m.blueTeam}`;
  const red =
    state.teams.find((t) => t.number === m.redTeam)?.name ??
    `Team ${m.redTeam}`;

...
export async function handleSlash(interaction: ChatInputCommandInteraction) {
  ...
  if (name === profileData.name) return profileExecute(interaction);
  if (name === lobbyData.name) return lobbyExecute(interaction);
}

/**
 * Route TOUTES les interactions (boutons, selects, modals, slash)
 */
export async function handleAny(interaction: Interaction) {
  ...
  if (interaction.isButton()) {
    ...
    if (stateByMsg && interaction.customId.startsWith("lobby:")) {
      return onButton(interaction as ButtonInteraction);
    }
    ...
    if (stateByChan && interaction.customId.startsWith("tb:")) {
      return onTeamBuilderButton(interaction as ButtonInteraction, stateByChan);
    }

    // 3) Match board (salon match)
    if (interaction.customId === MATCH_VALIDATE)
      return onMatchValidate(interaction as ButtonInteraction);
    if (interaction.customId === MATCH_REPOST)
      return onMatchRepost(interaction as ButtonInteraction);
    if (interaction.customId === MATCH_SKIP)
      return onMatchSkip(interaction as ButtonInteraction);
  ...
  if (interaction.isStringSelectMenu()) {
    return onSelect(interaction as StringSelectMenuInteraction);
  }

  if (interaction.isModalSubmit()) {
    return onModal(interaction as ModalSubmitInteraction);
  }
}

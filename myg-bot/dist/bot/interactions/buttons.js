...
import { get, toggleJoin, buildPoolFromSlots, ensureTeams, rememberState, // mapping channelId -> lobbyMessageId
 } from "../utils/lobby.js";
import { BTN_VALIDATE_ID, BTN_FAKE_ID, ROLES, TB_PREV, TB_NEXT, TB_SET_CAP, TB_NAME, TB_FORMAT, TB_FINALIZE, TB_SET_CAP_TEAM, TB_NAME_TEAM, } from "../utils/constants.js";
...
function idToRole(customId) {
    const match = customId.split(":");
    if (match[0] !== "lobby" || match[1] !== "join")
        return null;
    const role = match[2];
    return ROLES.includes(role) ? role : null;
}
// ===== Waiting room buttons (v1)
export async function onButton(i) {
    ...
    if (i.customId === BTN_FAKE_ID && state) {
        ...
        const addFake = (n, role) => {
            for (let k = 0; k < n; k++) {
                toggleJoin(state, role, `00000000000000${Math.floor(Math.random() * 1000)}`);
            }
        };
        ["Top", "Jungle", "Mid", "ADC", "Support", "Flex"].forEach((r) => addFake(1, r));
        addFake(3, "Sub");
        return i.update({
            embeds: [
                lobbyEmbed({
...
export async function onTeamBuilderButton(i, state) {
    if (!state)
        return;
    ...
    if (i.customId === TB_FORMAT) {
        const select = new StringSelectMenuBuilder()
            ...
            .addOptions({ label: "2 équipes — Bo1", value: "2t-bo1" }, { label: "2 équipes — Bo3", value: "2t-bo3" }, { label: "2 équipes — Bo5", value: "2t-bo5" }, { label: "4 équipes — 1 game (RR)", value: "4t-1g" }, { label: "4 équipes — 2 games (RR)", value: "4t-2g" }, { label: "4 équipes — 3 games (RR)", value: "4t-3g" }, { label: "6 équipes — 1 game (RR)", value: "6t-1g" }, { label: "6 équipes — 2 games (RR)", value: "6t-2g" }, { label: "6 équipes — 3 games (RR)", value: "6t-3g" });
    ...
    if (i.customId === TB_FINALIZE) {
        ...
        try {
            await (await import("../utils/lobby.js")).finalizeLobbyAndOpenFirstMatch(state, guild, matchChannelId);
            await i.editReply({
                content: "✅ Line-up envoyée + Match 1 lancé dans le salon match. Catégories d'équipes créées.",
            });
        }
        catch (e) {
            await i.editReply({

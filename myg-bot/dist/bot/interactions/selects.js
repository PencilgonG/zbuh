import { TB_SELECT_TEAM, TB_SELECT_ROLE, TB_SELECT_PLAYER, TB_SET_CAP_TEAM, TB_SET_CAP_PLAYER, TB_NAME_TEAM, } from "../utils/constants.js";
...
export function getStateForChannel(channelId) {
    if (!channelId)
        return undefined;
...
export async function onSelect(i) {
    ...
    if (i.customId === TB_SELECT_ROLE) {
        state.lastSelectedRole = i.values[0];
        return i.update(renderTeamBuilder(state));
    }
    ...
    if (i.customId === TB_SET_CAP_TEAM) {
        ...
        if (!team)
            return i.reply({ content: "Équipe inconnue.", ephemeral: true });
